...
const NewCategory = ({ lang }) => {
  const router = useRouter();
  const [name, setName] = useState({ es: "", en: "" });
  const [isSending, setIsSending] = useState(false);
  const [summary, setSummary] = useState({ es: "", en: "" });
  const [industryClients, setIndustryClients] = useState({ es: "", en: "" });
  const [benefits, setBenefits] = useState({ es: "", en: "" });
  const [precautions, setPrecautions] = useState({ es: "", en: "" });
  const [testSelection, setTestSelection] = useState(set_tests);
  const [typicalValueSelection, setTypicalValueSelection] =
    useState(set_typical_values);
                  ...
                  <div className="mb-4 w-full">
                    ...
                    <div className="relative">
                      <select
                        ...
                        className="appearance-none border border-gray-300 bg-gray-100 rounded-md pl-2 py-1 cursor-pointer focus:outline-none focus:border-gray-400 w-full"
                      >
                        {typicalValueSelection.map((option) => (
                          <option
                            key={option[`${lang}`]}
                            data-es={option.es}
                            data-en={option.en}
